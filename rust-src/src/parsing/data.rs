use super::bookmark::{option_checkout_result, Bookmark, CheckoutError};
...
impl ResponsiveTemplate<Bookmark> {
  pub fn checkout<'a>(self: Self, input: &'a str) -> CheckoutResult<ResponsiveTemplate<&'a str>> {
    match self {
      ...
      ResponsiveTemplate::Responsive(s) => {
        let mapped = try!(s.into_iter().map(|s| s.checkout(input)).collect());
  ...
  pub fn checkout_owned(self: Self, input: &str) -> CheckoutResult<ResponsiveTemplate<String>> {
    match self {
      ResponsiveTemplate::Normal => Ok(ResponsiveTemplate::Normal),
      ResponsiveTemplate::Responsive(s) => {
        let mapped = try!(s.into_iter().map(|s| s.checkout_owned(input)).collect());
        return Ok(ResponsiveTemplate::Responsive(mapped));
      }
    }
  }
...
impl ParseError {
  pub fn to_string(self: Self, input: &str) -> String {
    match self {
      ...
      ParseError::At(at, reason) => format!(
        "{} @ {} in {}",
        match reason {
          FailureReason::NotImplemented => "undocumented error".to_string(),

          FailureReason::ExpectedChar(c) => format!("expected {}", c.to_string()),

          FailureReason::ExpectedQuery => "expected 'media query'".to_string(),

          FailureReason::ExpectedValue => "expected 'value of query'".to_string(),

          FailureReason::ExpectedAs => "expected 'as-keyword'".to_string(),

          FailureReason::ExpectedOneOf(v) => format!("expected one of ({})", v.join(", ")),
